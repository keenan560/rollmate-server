...
app.get("/achievements/:userId", verifyToken, async (req, res) => {
  try {
    const { userId } = req.params;
    const currentUserId = req.user.uid;
    const page = parseInt(req.query.page, 10) || 1;
    ...
    // Check if current user has verified each achievement
...
// Create achievement
app.post("/achievements", verifyToken, async (req, res) => {
  try {
    const currentUserId = req.user.uid;
...
// Update achievement
app.put("/achievements/:achievementId", verifyToken, async (req, res) => {
  try {
...
// Delete achievement
app.delete("/achievements/:achievementId", verifyToken, async (req, res) => {
  try {
    const { achievementId } = req.params;
    const currentUserId = req.user.uid;
...
app.put("/profile/playing-style", verifyToken, async (req, res) => {
  try {
    const currentUserId = req.user.uid;
    const {
      playing_style,
